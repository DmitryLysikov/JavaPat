...
В проекте реализован паттерн Strategy для управления сортировкой задач. 

Он включает интерфейс TaskSortStrategy, который определяет метод sort для сортировки списка задач. 

Были созданы три стратегии: SortByDateStrategy для сортировки по дате выполнения, SortByPriorityStrategy для сортировки по приоритету и SortByTitleStrategy для сортировки по заголовку. 

Контекст TaskSorter использует текущую стратегию, задаваемую методом setStrategy, и выполняет сортировку через метод sortTasks. 

В меню добавлен пункт "Сортировать задачи", где пользователь выбирает стратегию сортировки, после чего задачи обрабатываются выбранным способом.

Задания 24 Template method

Паттерн Template Method реализован в проекте для упрощения и структурирования повторяющихся операций над задачами. 

Создан абстрактный класс TaskTemplate, который содержит шаблонный метод executeTaskAction().

В этом методе определена структура выполнения действия: логирование операции и вызов абстрактного метода performAction(), который реализуют подклассы.

Конкретные действия инкапсулированы в подклассах:

AddTaskTemplate — добавляет задачу в менеджер задач.

DeleteTaskTemplate — удаляет задачу по ID.

MarkTaskAsCompletedTemplate — отмечает задачу как выполненную.